...
async def list_books(
	...
	books = await BookService(BookRepository).get_all_books(page, per_page)
...
async def get_book(book_id: int):
	book = await BookService(BookRepository).get_by_id_book(book_id)
...
async def create_book(book_create: BookCreate):
	created_book = await BookService(BookRepository).create_book(book_create)
...
async def update_book(book_id: int, book_update: BookUpdate):
	updated_book = await BookService(BookRepository).update_book(book_id, book_update)
...
async def delete_book(book_id: int):
	await BookService(BookRepository).delete_book(book_id)
