...
class GenreService:
	...
	async def create_genre(self, genre_data: GenreCreate):
		if not await self.genre_repo.check_unique_genre_name_for_create(genre_data.name):
	...
	async def update_genre(self, genre_id: int, update_data: GenreUpdate):
		...
		if not await self.genre_repo.check_unique_genre_name_for_update(update_data.name, genre_id):
